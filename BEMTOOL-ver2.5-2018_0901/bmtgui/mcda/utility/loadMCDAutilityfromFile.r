...
loadMCDAutilityfromFile<-function(w) {
...
if (dialog$run() == GtkResponseType["accept"]) {
loaded_MCDAutility <- read.csv(dialog$getFilename(), sep=";", na.strings = "")  
vai <- T
dialog$destroy()
} else {
 vai <- F
...
#wnd <- showMessage("        Loading MCDA utility...        ")

MCDAutility_table <<- loaded_MCDAutility

#wnd$destroy()   
#gtkWidgetSetSensitive(BMTmain_window, TRUE)
wnd <- showMessageOK("        MCDA utility loaded!        ")
