...
# ------------------------------------------------------------------------------
#
bmt_OTHERS.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  bmt_OTHERS.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", bmt_OTHERS.cell_edited, bmt_OTHERS.model)
    month_frame <- data.frame(c(e))	
    colnames(month_frame) <- paste(" ", OTHERS_vector_names[e], " ", sep="")				       
  renderer$setData("column", month_frame)
  treeview$insertColumnWithAttributes(-1, paste(" ", OTHERS_vector_names[e], " ", sep="")	, renderer, text = e, editable = (length(OTHERS_vector) +1))
}
}