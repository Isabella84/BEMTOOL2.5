...
# ------------------------------------------------------------------------------
#
bmt_DAY.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  bmt_DAY.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", bmt_DAY.cell_edited, bmt_DAY.model)
    month_frame <- data.frame(c(e))	
    colnames(month_frame) <- paste(" ", MONTHS[e], " ", spe="")			       
  renderer$setData("column", month_frame)
  treeview$insertColumnWithAttributes(-1, paste(" ", MONTHS[e], " ", spe="")		, renderer, text = e, editable = 13)
}
}