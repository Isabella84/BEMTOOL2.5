...
# ------------------------------------------------------------------------------
#
price_costant_byfleet_discard.create_model <- function() {
#print("Creating model...")   
  # create list store
  price_costant_byfleet_discard.model <<- gtkListStoreNew("gchararray",  rep("gdouble", length(BMT_FLEETSEGMENTS)), "gboolean")  
  add.price_costant_byfleet_discard()
  # add items
  if (length(price_costant_byfleet_discard_list) != 0) { 
  for (i in 1:length(price_costant_byfleet_discard_list)) {
   ...
   # print(paste("in model:", years[nc]) )
         price_costant_byfleet_discard.model$set(iter, e, as.numeric(price_costant_byfleet_discard_list[[i]][e+1]))
    }
