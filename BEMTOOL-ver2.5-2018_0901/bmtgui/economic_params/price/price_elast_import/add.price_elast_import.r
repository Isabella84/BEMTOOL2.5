...
# ------------------------------------------------------------------------------
#
bmt_add.price_elast_import <- function() {
#print("Adding elements to the list...")   
  if (!is.null(bmt_fleet.price_elast_import)) {
  for (r in 1:nrow(bmt_fleet.price_elast_import)) {
  price_elast_import_temp <- as.list(bmt_fleet.price_elast_import[r,]) 
  names(price_elast_import_temp) <-  c("Species",BMT_FLEETSEGMENTS)
...
#print("price_elast_import (simulation) list successfully updated!", quote=F)
}