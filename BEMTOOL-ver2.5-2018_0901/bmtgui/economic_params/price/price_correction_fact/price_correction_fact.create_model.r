...
# ------------------------------------------------------------------------------
#
bmt_price_correction_fact.create_model <- function() {
#print("Creating model...")   
  # create list store
  bmt_price_correction_fact.model <<- gtkListStoreNew("gchararray",  rep("gdouble", length(BMT_FLEETSEGMENTS)), "gboolean")  
  bmt_add.price_correction_fact()
  # add items
  if (length(bmt_price_correction_fact_list) != 0) { 
  for (i in 1:length(bmt_price_correction_fact_list)) {
   ...
   # print(paste("in model:", years[nc]) )
         bmt_price_correction_fact.model$set(iter, e, as.numeric(bmt_price_correction_fact_list[[i]][e+1]))
    }
       bmt_price_correction_fact.model$set(iter, (length(BMT_FLEETSEGMENTS)+1),TRUE)
  } 
  }
  #print("price_correction_fact Model successfully created!")  
}
