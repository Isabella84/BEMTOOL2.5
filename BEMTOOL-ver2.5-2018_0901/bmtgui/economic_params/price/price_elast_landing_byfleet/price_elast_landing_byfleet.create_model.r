...
# ------------------------------------------------------------------------------
#
bmt_price_elast_landing_byfleet.create_model <- function() {
#print("Creating model...")   
  # create list store
  bmt_price_elast_landing_byfleet.model <<- gtkListStoreNew("gchararray",  rep("gdouble", length(BMT_FLEETSEGMENTS)), "gboolean")  
  bmt_add.price_elast_landing_byfleet()
  # add items
  if (length(bmt_price_elast_landing_byfleet_list) != 0) { 
  for (i in 1:length(bmt_price_elast_landing_byfleet_list)) {
   ...
   # print(paste("in model:", years[nc]) )
         bmt_price_elast_landing_byfleet.model$set(iter, e, as.numeric(bmt_price_elast_landing_byfleet_list[[i]][e+1]))
    }
