...
#  if (length(bmt_cost_fixed_list) != 0) { 
  for (i in 1:length(bmt_cost_fixed_list)) {
   ...
   # print(paste("in model:", years[nc]) )
         bmt_cost_fixed.model$set(iter, e, as.numeric(bmt_cost_fixed_list[[i]][e+1]))
    }
       bmt_cost_fixed.model$set(iter, (length(BMT_FLEETSEGMENTS)+1),TRUE)
  } else {
      bmt_cost_fixed.model$set(iter, 1,TRUE)
  } 
  }
#  }
  #print("cost_fixed Model successfully created!")  
}
