...
# ------------------------------------------------------------------------------
#
bmt_cost_fixed.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  bmt_cost_fixed.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", bmt_cost_fixed.cell_edited, bmt_cost_fixed.model)
