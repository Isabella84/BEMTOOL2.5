...
# ------------------------------------------------------------------------------
#
bmt_add.labour_fuel <- function() {
#print("Adding elements to the list...")   
  if (!is.null(bmt_fleet.labour_fuel)) {
  for (r in 1:nrow(bmt_fleet.labour_fuel)) {
  labour_fuel_temp <- as.list(bmt_fleet.labour_fuel[r,]) 
  names(labour_fuel_temp) <-  c(BMT_FLEETSEGMENTS)
  bmt_labour_fuel_list <<- c(bmt_labour_fuel_list, list(labour_fuel_temp)) 
  }
   } else {
   labour_fuel_matrix <- data.frame(matrix(0, nrow=1, ncol=length(BMT_FLEETSEGMENTS)))
...
#print("labour_fuel (simulation) list successfully updated!", quote=F)
}