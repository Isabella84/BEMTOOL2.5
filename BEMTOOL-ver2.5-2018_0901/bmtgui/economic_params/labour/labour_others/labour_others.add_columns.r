...
# ------------------------------------------------------------------------------
#
bmt_labour_others.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  bmt_labour_others.model <- treeview$getModel()
  # number column
...
#  treeview$insertColumnWithAttributes(-1, " " , renderer, text = 0, editable = FALSE) 
 ...
 renderer <- gtkCellRendererToggle()
  gSignalConnect(renderer, "toggled", bmt_labour_others.cell_edited, bmt_labour_others.model) 
 
 if (length(bmt_labour_others_list) != 0) {
    gtkCellRendererToggleSetActive(renderer,  bmt_labour_others_list[[1]][e][[1]])
 } 
#    if (value) {
#              bmt_labour_others.model$set(iter, e, TRUE)
#                gtkCellRendererToggleSetActive(renderer, T)
#         } else {
#              bmt_labour_others.model$set(iter, e, FALSE)
