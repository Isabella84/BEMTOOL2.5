...
# ------------------------------------------------------------------------------
#
bmt_indic_subsidies.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  bmt_indic_subsidies.model <- treeview$getModel()
  # number column
  renderer <- gtkCellRendererTextNew()
  # gSignalConnect(renderer, "edited", cell.edited, model)
  year_frame <- data.frame(c(0))	
  colnames(year_frame) <- c(" Fleet Segment ")			       
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", bmt_indic_subsidies.cell_edited, bmt_indic_subsidies.model)
