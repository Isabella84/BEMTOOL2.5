...
# ------------------------------------------------------------------------------
#
nofleetsegments.add_columns <- function(treeview) {
#print("Adding column to the model...")   
  nofleetsegments.model <- treeview$getModel()
   
  renderer <- gtkCellRendererTextNew()
  #renderer <- gtkCellRendererToggleNew()
  # gSignalConnect(renderer, "toggled", updateInteractions, nofleetsegments.model)
  param1_frame <- data.frame(c(0))
  param1_name <- " Fishing Technique "	
