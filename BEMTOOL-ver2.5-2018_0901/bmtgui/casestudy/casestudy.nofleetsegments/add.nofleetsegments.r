...
# ------------------------------------------------------------------------------
#
add.nofleetsegments <- function() {
#print("Adding elements to the list...")                               
  if (!is.null(nofleetsegments) ) {
  if ( nrow(nofleetsegments) != 0) {
  
  for (r in 1:nrow(nofleetsegments)) {
  dis_temp <- as.list(nofleetsegments[r,]) 

     heading <- c("Fleet segment")
 
  names(dis_temp) <- heading
  nofleetsegments_list <<- c(nofleetsegments_list, list(dis_temp)) 
  }
  }
   }
