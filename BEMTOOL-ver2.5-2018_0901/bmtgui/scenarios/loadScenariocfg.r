...
#     print(mat_cfg_TechProgress)
        ...
        ncol_indicators <- ifelse( (length(BMT_YEARS_FORECAST)+1) <= (length(indicator_cfg_names)+1), (length(indicator_cfg_names)+1), (length(BMT_YEARS_FORECAST)+1))
...
for (flee in 1:length(BMT_FLEETSEGMENTS)) {
  ...
  bmt_fleet.cost_equipment <<- indic_taxes_matrix
   ...
   matrix_temp <-   mat_cfg_EconomicIndicator[	(which(as.character(mat_cfg_EconomicIndicator[,1]) == "casestudy.discardCost.F1")-1):(which(as.character(mat_cfg_EconomicIndicator[,1]) == "casestudy.discardCost.F1")+(length(BMT_FLEETSEGMENTS))-1), ]
   ...
   colnames(discard_cost_matrix) <-  c("FleetSegment",BMT_YEARS_FORECAST)
     ...
     alLeastoneminusOne <- F
     
for (flee in 1:length(BMT_FLEETSEGMENTS)) {
      for (ye in 1:length(BMT_YEARS_FORECAST)) {
      
      coeff_disc_ <-  as.numeric(as.character(matrix_temp[which(as.character(matrix_temp[,1]) == paste("casestudy.discardCost.F", flee, sep="") ),(ye+1)] ))
      
      if (coeff_disc_ != -1 & !alLeastoneminusOne) {
       discard_cost_matrix[flee, ye+1] <- as.numeric(as.character(matrix_temp[which(as.character(matrix_temp[,1]) == paste("casestudy.discardCost.F", flee, sep="") ),(ye+1)] ))
       } else {
         alLeastoneminusOne <- T
         print("-1")
       }
}
}

if (!alLeastoneminusOne) {
...
gtkToggleButtonSetActive(radio_sorting_equalToDiscardRate, F)
   ...
   discard_cost_matrix[, 2:ncol(discard_cost_matrix)] <- -1
   bmt_fleet.discard_cost <<- discard_cost_matrix
              bmt_reload_discard_cost_table()
gtkToggleButtonSetActive(bmt_chk_economic_labour_sorting_coeff, T)
gtkToggleButtonSetActive(radio_sorting_equalToDiscardRate, F)
