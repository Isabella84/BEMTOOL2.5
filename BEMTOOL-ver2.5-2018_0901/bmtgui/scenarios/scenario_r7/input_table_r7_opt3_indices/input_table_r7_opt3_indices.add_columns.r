...
# ------------------------------------------------------------------------------
#
input_table_r7_opt3_indices.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  input_table_r7_opt3_indices.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", input_table_r7_opt3_indices.cell_edited, input_table_r7_opt3_indices.model)
    month_frame <- data.frame(c(1))	
    colnames(month_frame) <- " SSB index "		       
  renderer$setData("column", month_frame)
  treeview$insertColumnWithAttributes(-1, " SSB index ", renderer, text = 1, editable = 2)
