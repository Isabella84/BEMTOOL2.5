...
# ------------------------------------------------------------------------------
#
input_table_r7.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  input_table_r7.model <- treeview$getModel()
  # number column
  ...
  fields_temp <- c("% for the quota splitting")
  for (e in 1:length(fields_temp)) {
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", input_table_r7.cell_edited, input_table_r7.model)
