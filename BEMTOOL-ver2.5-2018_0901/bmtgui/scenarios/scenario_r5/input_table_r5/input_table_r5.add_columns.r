...
# ------------------------------------------------------------------------------
#
input_table_r5.add_columns <- function(treeview) {
# print("Adding column to the model...")   
  input_table_r5.model <- treeview$getModel()
  # number column
  ...
  fields_temp <- c("Reduction scenario *", "% vessel red.", "% days reduction")
  for (e in 1:length(fields_temp)) {
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", input_table_r5.cell_edited, input_table_r5.model)
