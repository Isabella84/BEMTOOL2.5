...
monthly.survivability.cell_edited <- function(cell, path.string, new.text, data) {
  ...
  #	print(monthly.survivability[[i]])
    	# monthly.survivability[[i]][column+1] <<- as.double(new.text)           # [column+1]     era così
  ...
  # 	monthly.survivability.model$set(iter, column, monthly.survivability[[i]][column+1])     era così
