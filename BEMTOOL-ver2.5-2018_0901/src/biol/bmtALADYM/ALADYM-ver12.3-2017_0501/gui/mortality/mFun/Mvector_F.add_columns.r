...
#
# ------------------------------------------------------------------------------
# Add the columns to to be rendered in the tree (FEMALES)
# ------------------------------------------------------------------------------
#
Mvector_F.add_columns <- function(treeview) {
#print("Adding column to the model...")   
  Mvector_F.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", Mvector_F.cell_edited, Mvector_F.model)
  ...
  #print("Natural mortality (FEMALES) column added with success!")
}
