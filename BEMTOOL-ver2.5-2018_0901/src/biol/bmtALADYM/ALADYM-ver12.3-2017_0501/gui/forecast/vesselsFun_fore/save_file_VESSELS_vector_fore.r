...
# ------------------------------------------------------------------------------
#
save_file_VESSELS_vector_fore <- function(w) {
...
VESSELS_fore_table <- get_table("VESSELS_FORE")

write.table(VESSELS_fore_table, dialog$getFilename(),  sep=";", na = "",row.names = FALSE)
dialog$destroy()
} else {
dialog$destroy()
}
