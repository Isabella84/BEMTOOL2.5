...
# ------------------------------------------------------------------------------
#
VESSELS_fore.add_columns <- function(treeview) {
#print("Adding column to the model...")   
 VESSELS_fore.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", VESSELS_fore.cell_edited, VESSELS_fore.model)
