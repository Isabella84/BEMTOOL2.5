...
# ------------------------------------------------------------------------------
#
add.SelectivityLength_M_fore <- function() {
...
#print("Adding elements to the list...")   
  ...
  names(FM_temp) <- c("Length", years_forecast )
 # names(FM_temp) <-  c(paste("age", c(first_age_mal:(n_ages+first_age_mal-1)), sep=""))
  SelectivityLength_M_fore_list <<- c(SelectivityLength_M_fore_list, list(FM_temp)) 
  }
   } else {
   FM_matrix <- data.frame(matrix(-1, nrow=length(l_inf_lens) , ncol=(length(years_forecast)+1)))
   colnames(FM_matrix) <- c("Length", years_forecast )
