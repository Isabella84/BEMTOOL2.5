...
if (is.na(as.double(new.text) )) {
  ...
  iter <- SelectivityLength_M_fore.model$getIter(path)$iter
   #print(paste("new text:", new.text))
  	i <- path$getIndices()[[1]]+1
  	#print(paste("indice i:", i))
  	#print(Zvector_M[[i]])
  	SelectivityLength_M_fore_list[[i]][column+1] <<- as.double(new.text)           # [column+1]
  #	print(paste("indice column:", column+1))
  #	print(Zvector_M[[i]][column+1])
  	SelectivityLength_M_fore.model$set(iter, column, SelectivityLength_M_fore_list[[i]][column+1])
