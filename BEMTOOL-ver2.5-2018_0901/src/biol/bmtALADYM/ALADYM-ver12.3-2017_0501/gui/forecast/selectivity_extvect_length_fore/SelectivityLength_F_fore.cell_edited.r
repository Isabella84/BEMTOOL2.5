...
SelectivityLength_F_fore.cell_edited <- function(cell, path.string, new.text, data) {
 if (is.na(as.double(new.text) )) {
  ...
  iter <- SelectivityLength_F_fore.model$getIter(path)$iter
   #print(paste("new text:", new.text))
  	i <- path$getIndices()[[1]]+1
  	#print(paste("indice i:", i))
  	#print(Zvector_M[[i]])
  	SelectivityLength_F_fore_list[[i]][column+1] <<- as.double(new.text)           # [column+1]
  #	print(paste("indice column:", column+1))
  #	print(Zvector_M[[i]][column+1])
  	SelectivityLength_F_fore.model$set(iter, column, SelectivityLength_F_fore_list[[i]][column+1])
