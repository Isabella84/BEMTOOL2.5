...
export_lan_obligationtoFile_fore<-function(w) {
...
lanObl_table <- data.frame(matrix(nrow=(length(years_forecast)*12), ncol=4 ))
...
colnames(lanObl_table) <- heading
 ...
 years_rep <- rep(years_forecast, 12)
   ...
   landObl_fs <- data.frame(FleetList_forecast[[fs]]@landing.obligation.vector)

   if (nrow(landObl_fs) > 0) {
for (yea in years_forecast) {
