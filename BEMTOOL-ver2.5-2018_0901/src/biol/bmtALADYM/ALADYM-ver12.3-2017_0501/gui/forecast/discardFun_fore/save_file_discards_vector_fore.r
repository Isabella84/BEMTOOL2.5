...
# ------------------------------------------------------------------------------
#
save_file_discards_vector_fore <- function(w) {
...
discard_fore_table <- get_table("DISCARD_FORE")
write.table(discard_fore_table, dialog$getFilename(),  sep=";", na = "",row.names = FALSE)
dialog$destroy()
} else {
dialog$destroy()
}
