...
# ------------------------------------------------------------------------------
#
save_file_FISHINGEFFORT_vector_fore <- function(w) {
...
FISHINGEFFORT_fore_table <- get_table("FISHING_EFFORT_FORE")

write.table(FISHINGEFFORT_fore_table, dialog$getFilename(),  sep=";", na = "",row.names = FALSE)
dialog$destroy()
} else {
dialog$destroy()
}
