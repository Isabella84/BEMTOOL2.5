...
# ------------------------------------------------------------------------------
#
FishingMvector_M_fore.add_columns <- function(treeview) {
  first_age_mal <- 0
...
#print("Adding column to the model...")   
  FishingMvector_M_fore.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", FishingMvector_M_fore.cell_edited, FishingMvector_M_fore.model)
