...
# ------------------------------------------------------------------------------
#
reload_EMPTY_discard_extvector_M_fore <- function(w) {
...
if (!IN_BEMTOOL | (IN_BEMTOOL & phase=="SIMULATION") ) {
  ...
  discards_extvector_M_list_fore <<- c(discards_extvector_M_list_fore, list(FF_temp)) 
  }
  fleet.discard_extvector_M_fore  <<- FF_matrix
  ...
  for (i in 1:length(discards_extvector_M_list_fore)) {
   ...
   # print(paste("in model:", years[nc]) )
         discards_extvector_M_fore.model$set(iter, e, as.numeric(discards_extvector_M_list_fore[[i]][e+1]))
    }
...
reload_discard_extvector_M_fore <- function(w) {
...
if (!IN_BEMTOOL | (IN_BEMTOOL & phase=="SIMULATION") ) {
  ...
  n_ages <- as.numeric(new_aldPopulation@lifespan[2,1])      
    ...
    first_age_mal <- trunc(Tr/12)
       ...
       FF_matrix <-   fleet.discard_extvector_M_fore
   for (r in 1:nrow(FF_matrix)) { 
  FF_temp <- as.list(FF_matrix[r,]) 
  discards_extvector_M_list_fore <<- c(discards_extvector_M_list_fore, list(FF_temp)) 
  }
