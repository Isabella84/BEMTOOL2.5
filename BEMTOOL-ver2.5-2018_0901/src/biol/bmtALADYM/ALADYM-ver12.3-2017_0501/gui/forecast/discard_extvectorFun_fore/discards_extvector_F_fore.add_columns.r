...
# ------------------------------------------------------------------------------
#
discards_extvector_F_fore.add_columns <- function(treeview) {
...
#print("Adding column to the model...")   
  discards_extvector_F_fore.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", discards_extvector_F_fore.cell_edited, discards_extvector_F_fore.model)
    month_frame <- data.frame(c(e))	
    colnames(month_frame) <- c(paste("age", (e+first_age_fem-1), sep=""))			       
  renderer$setData("column", month_frame)
  treeview$insertColumnWithAttributes(-1, as.character(paste("age", (e+first_age_fem-1), sep="")), renderer, text = e, editable = (n_ages+1))
}
}
