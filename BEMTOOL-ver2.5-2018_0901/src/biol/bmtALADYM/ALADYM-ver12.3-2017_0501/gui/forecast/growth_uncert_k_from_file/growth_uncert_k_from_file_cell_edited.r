...
# ------------------------------------------------------------------------------
#
growth_uncert_k_from_file_cell_edited <- function(cell, path.string, new.text, data) {
  
  #checkPtrType(data, "GtkListStore")
  growth_uncert_k_from_file_model <- data 
  ...
  iter <- growth_uncert_k_from_file_model$getIter(path)$iter
   #print(paste("new text:", new.text))
  	i <- path$getIndices()[[1]]+1
  #	print(paste("indice i:", i))
  	#print(recruitments_fore_from_vector[[i]])
  	growth_uncert_k_from_file_list[[i]][column+1] <<- as.double(new.text)           # [column+1]
  #	print(paste("indice column:", column+1))
  #	print(recruitments_fore_from_vector[[i]][column+1])
  	growth_uncert_k_from_file_model$set(iter, column, growth_uncert_k_from_file_list[[i]][column+1])
}
