...
# ------------------------------------------------------------------------------
#
escape_survival_extvector_M_fore.add_columns <- function(treeview) {
...
#print("Adding column to the model...")   
  escape_survival_extvector_M_fore.model <- treeview$getModel()
  # number column
  ...
  # number column
  renderer <- gtkCellRendererTextNew()
  gSignalConnect(renderer, "edited", escape_survival_extvector_M_fore.cell_edited, escape_survival_extvector_M_fore.model)
    month_frame <- data.frame(c(e-1))	
    colnames(month_frame) <- c(paste("age", (e+first_age_mal-1), sep=""))			       
  renderer$setData("column", month_frame)
  treeview$insertColumnWithAttributes(-1, as.character(paste("age", (e+first_age_mal-1), sep="")), renderer, text = (e-1), editable = (n_ages))
}
}
