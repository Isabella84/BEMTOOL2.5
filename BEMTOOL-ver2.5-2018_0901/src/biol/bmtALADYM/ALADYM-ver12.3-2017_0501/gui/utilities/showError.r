...
showError<-function(error_mess){
error_window <<- gtkWindow(show=FALSE)       
if (!IN_BEMTOOL) {       
error_window["title"] <- "ERROR - ALADYM v11"  
} else {
error_window["title"] <- "ERROR - BEMTOOL 2.0"  
}  
gtkWindowSetModal(error_window, TRUE)         
gtkWindowSetDefaultSize(error_window, 400, 50)
...
vbox$PackStart(hbox1, expand = TRUE, fill = FALSE, padding = 35)
vbox$PackStart(hbox2, expand = TRUE, fill = FALSE, padding = 30)
