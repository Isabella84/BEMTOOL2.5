...
update_BMT_fleetsegments_fore <- function(num_ORD_fleetsegm, num_fleet) {
                           # num_fleet <- 1 from bemtool
                         # num_ORD_fleetsegm <- 1 from ALADYM 
...
red_value <- as.numeric(as.character(cfg[rownames(cfg) == paste("casestudy.HR3.F", num_fleet , sep=""),1]))
if (red_value == 1) {
 FleetList_forecast[[num_ORD_fleetsegm]]@scenario.reduction <<-  "Exponential"
} else if (red_value == 2) {
 FleetList_forecast[[num_ORD_fleetsegm]]@scenario.reduction <<-  "Linear"
} else {
 FleetList_forecast[[num_ORD_fleetsegm]]@scenario.reduction <<-  "Logistic"
}
}
...
GLO$L_number <- simperiod * 12 + foreperiod*12  
