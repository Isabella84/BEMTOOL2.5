...
saveSelectivitytoFile<-function(w) {
...
# save_selectivity_path = "C:\\FACCHINI_MT\\ALADYM - sw\\ALADYM-ver10.0.3a-2014_2502\\saved_selectivity.csv"
...
#selectivity_table <- data.frame(matrix(nrow=(length(years)*12)+1, ncol=8 ))

 #if (table_sel_params$n_par == 2) {
#   heading <- c("year","month",  as.character(table_sel_params$param1),  as.character(table_sel_params$param2))
# } else if (table_sel_params$n_par == 3) {
#   heading <- c("year","month",   as.character(table_sel_params$param1),  as.character(table_sel_params$param2),  as.character(table_sel_params$param3))
# } else if (table_sel_params$n_par == 5) {
#   heading <- c("year","month",   as.character(table_sel_params$param1),  as.character(table_sel_params$param2),  as.character(table_sel_params$param3),  as.character(table_sel_params$param4),  as.character(table_sel_params$param5) )
# } 
#colnames(selectivity_table) <- c("year","month", "param1", "param2", "param3", "param4", "param5", "sel_type" )
#
...
#to_add <- data.frame(matrix(NA, nrow=nrow(selectivity_table), ncol=(5-table_sel_params$n_par)))
