...
# ------------------------------------------------------------------------------
#
add.escape_survival_extvector_M <- function() {
...
#print("Adding elements to the list...")   
  if (!is.null(escape_surv_extvector_Mtable)) {
  for (r in 1:nrow(escape_surv_extvector_Mtable)) {
  FF_temp <- as.list(escape_surv_extvector_Mtable[r,]) 
  # names(FF_temp) <- c("year",paste("age", c(0:(n_ages-1)), sep=""))
    names(FF_temp) <-  c(paste("age", c(first_age_mal:(n_ages+first_age_mal-1)), sep="") )
