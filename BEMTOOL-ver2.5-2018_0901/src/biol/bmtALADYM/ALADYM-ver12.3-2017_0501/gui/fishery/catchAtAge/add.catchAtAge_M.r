...
#print("Adding elements to the list...")   
  if (!is.null(catchAtAge.vector.males)) {
  for (r in 1:nrow(catchAtAge.vector.males)) {
  FM_temp <- as.list(catchAtAge.vector.males[r,]) 
  # names(FM_temp) <- c("year", paste("age", c(0:(n_ages-1)), sep="") )
  names(FM_temp) <-  c("year",paste("age", c(first_age_mal:(n_ages+first_age_mal-1)), sep=""))
  catchAtAgeM_list <<- c(catchAtAgeM_list, list(FM_temp)) 
  }
   } else {
   FM_matrix <- data.frame(matrix(-1, nrow=length(years), ncol=(n_ages+1)))
