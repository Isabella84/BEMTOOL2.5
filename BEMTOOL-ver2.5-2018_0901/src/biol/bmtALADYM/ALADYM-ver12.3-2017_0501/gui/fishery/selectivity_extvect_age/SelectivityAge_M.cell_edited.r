...
if (is.na(as.double(new.text) )) {
  ...
  iter <- SelectivityAge_M.model$getIter(path)$iter
   #print(paste("new text:", new.text))
  	i <- path$getIndices()[[1]]+1
  	#print(paste("indice i:", i))
  	#print(Zvector_M[[i]])
  	SelectivityAge_M_list[[i]][column+1] <<- as.double(new.text)           # [column+1]
  #	print(paste("indice column:", column+1))
  #	print(Zvector_M[[i]][column+1])
  	SelectivityAge_M.model$set(iter, column, SelectivityAge_M_list[[i]][column+1])
