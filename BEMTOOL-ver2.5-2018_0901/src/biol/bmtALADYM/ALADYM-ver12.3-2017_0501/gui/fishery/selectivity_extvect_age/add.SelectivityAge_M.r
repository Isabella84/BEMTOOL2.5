...
#print("Adding elements to the list...")   
  ...
  for (r in 1:nrow(SelectivityAgeM_matrix)) {
  FM_temp <- as.list(SelectivityAgeM_matrix[r,]) 
 ...
 # names(FM_temp) <-  c(paste("age", c(first_age_mal:(n_ages+first_age_mal-1)), sep=""))
  SelectivityAge_M_list <<- c(SelectivityAge_M_list, list(FM_temp)) 
  }
   } else {
