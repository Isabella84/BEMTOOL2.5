...
# ------------------------------------------------------------------------------
#
lan_obligation.cell_edited <- function(cell, path.string, new.text, data) {
  #checkPtrType(data, "GtkListStore")

  lan_obligation.model <- data 
  ...
  iter <- lan_obligation.model$getIter(path)$iter
   #print(paste("new text:", new.text))
  	i <- path$getIndices()[[1]]+1
  	#print(paste("indice i:", i))
  #	print(pproductions[[i]])
  	lan_obligation[[i]][column+1] <<- new.text           # [column+1]
  #	print(paste("indice column:", column+1))
  #	print(pproductions[[i]][column+1])
  	lan_obligation.model$set(iter, column, lan_obligation[[i]][column+1])
