...
# ------------------------------------------------------------------------------
#
FishingMvector_M_overall.cell_edited <- function(cell, path.string, new.text, data) {
if (is.na(as.double(new.text) )) {
      showError("Value for fishing mortality must be a number!")
 } else {  
  #checkPtrType(data, "GtkListStore")
  FishingMvector_M_overall.model <- data 
  ...
  iter <- FishingMvector_M_overall.model$getIter(path)$iter
   #print(paste("new text:", new.text))
  	i <- path$getIndices()[[1]]+1
  	#print(paste("indice i:", i))
  	#print(Zvector_M[[i]])
  	FishingMvector_M_overall_list[[i]][column+1] <<- as.double(new.text)           # [column+1]
  #	print(paste("indice column:", column+1))
  #	print(Zvector_M[[i]][column+1])
  	FishingMvector_M_overall.model$set(iter, column, FishingMvector_M_overall_list[[i]][column+1])
  	
  	#new_aldSimulation@fishingmortality.overall.M <<- 
  	}
}
