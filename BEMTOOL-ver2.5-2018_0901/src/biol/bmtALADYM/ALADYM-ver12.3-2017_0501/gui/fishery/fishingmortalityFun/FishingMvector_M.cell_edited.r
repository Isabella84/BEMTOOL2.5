...
if (is.na(as.double(new.text) )) {
  ...
  iter <- FishingMvector_M.model$getIter(path)$iter
   #print(paste("new text:", new.text))
  	i <- path$getIndices()[[1]]+1
  	#print(paste("indice i:", i))
  	#print(Zvector_M[[i]])
  	FishingMvector_M[[i]][column+1] <<- as.double(new.text)           # [column+1]
  #	print(paste("indice column:", column+1))
  #	print(Zvector_M[[i]][column+1])
  	FishingMvector_M.model$set(iter, column, FishingMvector_M[[i]][column+1])
  	
  	FleetList_simulation[[gtkComboBoxGetActive(combo_fleetsegments)+1]]@fishingmortality.M.vector <<- get_table("FISHING_MORTALITY_M")
  	}
}
