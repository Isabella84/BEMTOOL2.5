...
#
#
change_maturityogive_L50_M <-function(w){
          gtkWidgetSetSensitive(lbl_A_maturityogive_L50_M, TRUE)
          gtkWidgetSetSensitive(entryOGIVEL50_M_a, TRUE)
          gtkWidgetSetSensitive(lbl_B_maturityogive_L50_M, TRUE)
          gtkWidgetSetSensitive(entryOGIVEL50_M_b, TRUE)

  select_index = -1
  selected <- gtkComboBoxGetActiveText(combo_L50dis_M)
...
# print(paste("Selected element: ", selected, "[",select_index,"]", sep=""))
      ...
      if (select_index == 1) {
          ...
          gtkWidgetSetSensitive(entryOGIVEL50_M_a, FALSE)
          gtkWidgetSetSensitive(lbl_B_maturityogive_L50_M, FALSE)
          gtkWidgetSetSensitive(entryOGIVEL50_M_b, FALSE)
          lbl_A_maturityogive_txt <- "A"
          lbl_B_maturityogive_txt <- "B"
      }  
gtkLabelSetText(lbl_A_maturityogive_L50_M, lbl_A_maturityogive_txt)
gtkLabelSetText(lbl_B_maturityogive_L50_M, lbl_B_maturityogive_txt)
}