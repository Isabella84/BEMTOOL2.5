...
P_production_calc <- function (prod_data,n) {
# gear_data=read.table(file=Gears_data,sep=";", header=TRUE)
#prod_data= read.table(prod_data_name,header=TRUE,sep=";")      
# nb_gears=2
Production =  matrix(nrow=(n),ncol=nb_gears)
Proportions =  matrix(nrow=(n),ncol=nb_gears)
...
#    Proportions[r,c]<- ifelse(sum(Production [r,])==0,0,Production [r,c]/sum(Production [r,]))
