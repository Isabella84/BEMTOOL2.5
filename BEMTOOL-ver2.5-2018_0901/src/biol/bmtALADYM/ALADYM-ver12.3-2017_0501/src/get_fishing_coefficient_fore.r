...
get_fishing_coefficient_fore<-function() {

# Table of effort data (Gear	Month	Vessels	Days	GT )   es: Trawler1	0	20	300	10
fc_all <- data.frame(matrix(nrow=(length(c(years, years_forecast))*12)+1, ncol=length(FLEETSEGMENTS_names)))
...
for (yy in 1:length(years_forecast)) {
        ...
        fc_all[(yy + length(years)-1)*12 + c(2:13),n_eff] <- as.numeric( as.character(FleetList_forecast[[n_eff]]@fishingeffort.vector[yy, 2:13] ))
} # end year
