...
# 
#  if (nb_sheets!= 1) {
#nome_file <- paste(F_BYGEAR_graph,"_sheet", i,".jpg", sep="")
#} else {
#nome_file <- paste(F_BYGEAR_graph,".jpg", sep="")
#}
# if (nb_gears <= 4) {
...
#          for (m in (8*i-7):(8*(i-1)+(nb_loops))) {
     ...
     if (m==1) {
nome_file <- paste(F_BYGEAR_graph,"_total.jpg", sep="")
} else {
nome_file <- paste(F_BYGEAR_graph,"_", m-1,".jpg", sep="")
}    
...
Ylim=c(Ymin,(Ymax+(Ymax-Ymin)/3) ) 
           ...
           plot(loca_xa, mortality$Annual_F_estimated,   type="l",  main="Total F", xlab="Time_slice [year]", ylab="F[year-1]", ylim=Ylim,lwd=2, col="blue") 
           points(loca_xa, mortality$Annual_F_estimated,   type="p",  pch=20,  col="blue") 
...
plot(loca_xa,mortality[,which(colnames(mortality)==as.character(Fattrezzi[m-1]))],type="l",   main=paste("F for",as.character(FLEETSEGMENTS_names[m-1])), xlab = "Time slice [year]", ylab = "F[year-1]",lwd = 2, col="blue",ylim=Ylim)  
points(loca_xa,mortality[,which(colnames(mortality)==as.character(Fattrezzi[m-1]))],type="p",   pch=20, col="blue") 
 mtext( GLO$ThisIsVersion,side=4,outer=FALSE)
...
#	  legend ("topleft", lwd=c(2,1,1),lty=c(1,2,3),legend=c("median", "0.05", "0.95"),col=c(m,"black","black"), bty="n" )
    ...
    plot(loca_xa,mortality$Annual_F_estimated,type="l",  main=paste("F for",as.character(FLEETSEGMENTS_names[m-1])), xlab = "Time slice [year]", ylab = "F[year-1]",lwd = 2, col="blue",ylim=Ylim) 
    points(loca_xa,mortality$Annual_F_estimated,type="p",   pch=20, col="blue") 
               mtext( GLO$ThisIsVersion,side=4,outer=FALSE)
