...
for (g in 1:nb_gears) {
 ...
 # prima: se il modello di selettività non è mai cambiato negli ultimi x mesi, prendi una media dei parametri
...
INP$Land_obl  <- matrix(nrow = loca_Nsimulation, ncol = nb_gears)  
...
if ( all(INP$OPT_SG_TYPE[(forecast-INP$Average_forecast*12+1):forecast,g] == INP$OPT_SG_TYPE[forecast,g]) ) {
 ...
 if (FleetList_simulation[[g]]@discard.calculation == "YES") { 
  if (FleetList_simulation[[g]]@discard.datatype == "Reverse ogive") {
    INP$Type_discard[i,g]  <- 1
    INP$param62[i,g]  <-  mean(INP$param6[(forecast-INP$Average_forecast*12+1):forecast,g]) #  as.numeric(as.character(FleetList_simulation[[gear]]@discard.vector[loca_i, 3] ))
    INP$param72[i,g]  <- mean(INP$param7[(forecast-INP$Average_forecast*12+1):forecast,g]) # as.numeric(as.character(FleetList_simulation[[gear]]@discard.vector[loca_i, 4] )) 
   } else {
    INP$Type_discard[i,g] <- 2
    INP$param62[i,g] <- NA
    INP$param72[i,g] <- NA
   }
  }
...
INP$Discard2[i,g] <- ifelse(disc_calc == "YES", "Y", ifelse(disc_calc == "0", 0, NA))

INP$Land_obl[i,g] = INP$Land_obl[nrow(INP$Land_obl),g]
...
# INP$Land_obl  [i,g] = INP$Land_obl [forecast,g]
...
#  }
  ...
  INP$Fishing_efforts2[i,g] <- INP$Fishing_efforts[nrow(INP$Fishing_efforts),g]
  INP$Discard2[i,g] <- INP$Discard[nrow(INP$Discard),g]
  INP$param62[i,g] <- INP$param6[nrow(INP$param6),g]
  INP$param72[i,g] <- INP$param7[nrow(INP$param7),g]
  if (as.numeric(INP$OPT_F_TYPE)==1) { 
  INP$p_Production2[i,g] <-INP$p_Production[nrow(INP$param7),g]
  }
