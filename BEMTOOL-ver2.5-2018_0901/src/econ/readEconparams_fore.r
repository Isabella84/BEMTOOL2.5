...
discard_coeff_mat <<- data.frame(matrix(0, nrow=n_fleet, ncol=foreperiod))
...
for (n in 1:n_fleet) {
    for (ti in 1:foreperiod) {
        ...
        discard_coeff_mat[[n,ti]] = as.character(cfg[rownames(cfg) == paste("casestudy.discardCost.F", n,sep=""),ti])
    }
}
